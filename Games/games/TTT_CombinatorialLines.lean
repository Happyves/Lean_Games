...
/-
This file contains API surrounding combinatorial lines,
which is used in high-dimensional Tic-tac-toe.

The main concepts are:
- `is_combi_line` describes what it means for a finite set of points
  to be a combinatorial line
- `count_below` shows that a line
- `incidence_ub` upper bounds the number of ombinatoriallines passing through
  a given point of the grid by `(3^D - 1)//2`
-/

variable (D n : ℕ) -- D is the dimension, n the grid-cube-length
...
/-- The opposite, in the sense that `Opp 0 = n-1`, `Opp 1 = n-2`, ..., `Opp (n-1) = 0`-/
...
/--
A sequence `s` ofn of number in {0,...,n-1} is
- `cst` : constant if there an x ∈ {0,...,n-1} such that all s i = x
- `inc` : increasing if for all i, s i = i
- `dec` : decreasing if for all i, s i = Opp i, ie. s 0 = n-1, s 1 = n-2, ..., s (n-1) = 0
-/
...
/--
Recall that we consider `(Fin D → Fin n)` to be {0,...,n-1}^D,
and `(s : Fin n → (Fin D → Fin n))` to be a sequence of such points,
indexed by {0,...,n-1}.

Such a sequence represents a combinatorial line if:
- in all dimensions, the coordinates for increasing, decreasing or constant sequences
- not all coordintes from constant sequneces (otherwise, it'd be a point, not a line)
-/
...
/--
A sequence represents a line if the sequence is a line and it's image is the set.
-/
...
/-- A set is a combinatorial line if there is a certain sequence representing it-/
