...
def PickUpBricks : Symm_Game_World ℕ ℕ where
  ...
  fst_win_states := (fun ini hist => bricks_from_ini_hist ini hist = 0) -- won once no bricks left
  snd_win_states := (fun ini hist => bricks_from_ini_hist ini hist = 0)
  transition := fun ini => bricks_from_ini_hist_act ini
  law := fun ini hist act =>  let B := (bricks_from_ini_hist ini hist)
                              if B = 0
                              then act = 0
                              else  if B = 1
                                    then act = 1
...
def pub_win_strat : fStrategy init_bricks (PickUpBricks init_bricks).law (PickUpBricks init_bricks).law :=
  fun hist _ _ =>
...
def toy_strat : sStrategy init_bricks (PickUpBricks init_bricks).law (PickUpBricks init_bricks).law :=
  fun hist _ _ =>
    let B := bricks_from_ini_hist init_bricks hist
    if Z : B = 0
    then ⟨0, (by dsimp [PickUpBricks] ; dsimp [B] at Z ; rw [Z, if_pos rfl])⟩
...
def PickUpBricks_pubWin_vs_toy : Symm_Game ℕ ℕ :=
  {(PickUpBricks 37) with
   fst_strat := pub_win_strat 37
   snd_strat := toy_strat 37
  }


#eval (PickUpBricks_pubWin_vs_toy.history_on_turn 30).val.reverse
...
lemma loop_invariant
  ...
  (s_strat : sStrategy init_bricks (PickUpBricks init_bricks).law (PickUpBricks init_bricks).law) :
  let g : Symm_Game ℕ ℕ := { (PickUpBricks init_bricks) with
                             fst_strat := pub_win_strat init_bricks
                             snd_strat := s_strat
                            } ;
